...
resource "ovh_cloud_project_user" "s3_users" {
  ...
  description  = "Cree par Terraform pour acces s3 ${lower(var.s3.bucket_name)}: ${each.value.description}"
    ...
    Statement = [
      {
        ...
        Resource = [
          for path in each.value.resources :
            path == "*"
              ? "arn:aws:s3:::${var.bucket.bucket_name}/*"
              : "arn:aws:s3:::${var.bucket.bucket_name}/${trimprefix(path, "/")}"
        ]
      }
    ]
  })
